...
function clickRight() {
    ...
    if (currentLeft < -810 && screen.width < 420) { //si el valor de izquierda es menor a -810, para de mover el contenido
        return;
    }
    if (currentLeft < -270 && screen.width > 420) { //si el valor de izquierda es menor a -270, para de mover el contenido
        return;
    }
...
function closeModal(e) {
    if (
        e.target.className.includes("modal-project-image")
    ) {
...
function handleSubmit(e) {
    ...
    mailanchor.setAttribute("href", `mailto:fercho.rod16@gmail.com?subject=${form.get('name')} ${form.get('email')}&body=${form.get('message')}`);
    mailanchor.click();
}